function recommendBooksV2() {
  let element = document.getElementById("force_graph_2");

  if (element != null) {
    element.remove();
  }

  let query = document.getElementById("user_query_2");

  if (query.value === "") {
    alert("Please enter some text before querying!");
  } else {
    const url = `http://localhost:5000/recommend_user/${query.value}`;
    fetch(url)
